...
function randomSynArray(num_neurons)
	...
	S = (rand(0:50, num_neurons, num_neurons))
	return S
end
...
function timeDifOfNear(PreSpikes, PostSpikes)
	sum_delay = 0
	n = 0
	for t in (look_around+101):(length(PreSpikes)-look_around) #searches presynaptic spikes, ignores early time before net could be expect to know
...
function STDP()
	...
	println(ArrayOfEXIN)
	save("C:/Users/Dominic/Documents/julia projects/STDPexin.jld", "exin", ArrayOfEXIN)

	l = length(allowedSpaces)/2
	learning_rate = 0.1/150
	...
	for gen in 1:num_gens
		...
		for pick in 1:150
			...
			if maximum(counts) <= sim_length/3 && maximum(counts) != 0
				counts/=(maximum(counts))
				learnDir = rightGuess(counts, ans) #will strengthen connections hebbian if guesses right, will do the opposite if wrong
			else
				learnDir = 0
			end
			...
			for idx in 1:l
				...
				#=
				if j > num_neurons - num_out
					t = timeDifOfNear(allSpikes[i, :], refSpikes[num_neurons - j + 1, :])
				else
				...
				if t < 0
					delta[i, j] += learning_rate*exp(t/look_around)*ArrayOfNeurs[i].exin*learnDir*genadj
				elseif t > 0
					delta[i, j] += learning_rate*exp(-t/look_around)*ArrayOfNeurs[i].exin*learnDir*genadj
				end
	...
	println(Brain)
	save("C:/Users/Dominic/Documents/julia projects/STDPbrain.jld", "winner", Brain)
end
